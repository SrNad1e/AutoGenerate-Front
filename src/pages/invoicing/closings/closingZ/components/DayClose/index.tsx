...
const CloseDay = ({ visible, onCancel, cashRegister }: Props) => {
  ...
  const quantityDataphone = data?.createCloseZInvoicing?.payments?.reduce(
    (sum, payment) =>
      sum + (payment?.payment?.type === TypePayment.Dataphone ? payment?.quantity : 0),
    0,
  );

  const diffDataphone = data?.createCloseZInvoicing?.quantityDataphone - quantityDataphone;
            ...
            const response = await createClose({
              variables: {
                input: {
                  ...
                  quantityDataphone: values2?.quantityDataphone,
                },
              },
            });
      ...
      >
        ...
        {currentStep === 1 && (
          <>
            ...
            >
              <InputNumber defaultValue={0} controls={false} style={styles.inputWidth} />
            </FormItem>
          </>
        )}
        {currentStep === 2 && (
          <Row justify="center">
            ...
            >
              <Text>{data?.createCloseZInvoicing?.quantityDataphone}</Text>
            </Col>
